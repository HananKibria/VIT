"""
STAGE 1: SurgVU Pre-training (Standalone Script with Recursive Video Discovery)
================================================================================
...
UPDATED: Now supports nested folder structures - automatically finds all .mp4 files
in any subdirectory within the SurgVU root folder.
...
# ============================================================================
# SURGVU DATASET IMPLEMENTATION (WITH RECURSIVE FOLDER SEARCH)
# ============================================================================

class SurgVUDataset(Dataset):
    """
    Complete SurgVU dataset implementation with RECURSIVE video discovery.
    
    Now supports videos organized in nested folders:
    data/SurgVU/
    ├── category1/
    │   ├── video1.mp4
    │   └── video2.mp4
    ...
    All .mp4 files will be found automatically regardless of folder structure.
    """
    ...
    def __init__(
        ...
        self.video_files = self._discover_videos_recursive()
        
        if len(self.video_files) == 0:
            raise ValueError(
                f"No .mp4 files found in {self.root} or its subdirectories!\n"
                f"Please check that:\n"
                f"  1. The path is correct: {self.root.absolute()}\n"
                f"  2. Video files have .mp4 extension\n"
                f"  3. You have read permissions"
            )
        
        print(f"Found {len(self.video_files)} video files across all subdirectories")
    ...
    def _discover_videos_recursive(self):
        """
        Recursively discover all video files in the dataset directory.
        
        Searches through all subdirectories and collects .mp4 files.
        """
        video_files = []
        
        # Use rglob to recursively find all .mp4 files
        mp4_files = list(self.root.rglob('*.mp4'))
        
        # Also try .MP4 (uppercase)
        MP4_files = list(self.root.rglob('*.MP4'))
        
        video_files = mp4_files + MP4_files
        
        if len(video_files) == 0:
            print(f"⚠ No video files found!")
            print(f"Searched in: {self.root.absolute()}")
            print(f"\nDirectory structure:")
            try:
                for item in self.root.rglob('*'):
                    if item.is_file():
                        print(f"  Found file: {item.relative_to(self.root)}")
            except:
                print("  Could not list directory contents")
        else:
            print(f"✓ Found {len(video_files)} video files:")
            # Show first few files and their locations
            for i, vf in enumerate(video_files[:5]):
                rel_path = vf.relative_to(self.root)
    ...
    def _create_samples(self):
        ...
        for video_file in tqdm(self.video_files, desc="Processing videos"):
            # Get video info
            ...
            if total_frames == 0 or fps == 0:
                print(f"⚠ Warning: Invalid video {video_file.name}, skipping...")
                continue
            
            # Calculate sampling stride
            frame_stride = max(1, fps // self.sample_fps)
        ...
        print(f"✓ Created {len(samples)} training clips")
        return samples
...
def main():
    ...
    parser.add_argument('--data_root', type=str, required=True,
                        help='Path to SurgVU dataset root directory (will search all subdirectories)')
    ...
    print(f"Searching for videos recursively in: {args.data_root}")
